...
public class LocalEditDistance {
    ...
    private LocalEditDistance(int match, int insert, int delete, int replace) {
        this.match = match;
        this.insert = insert;
        this.delete = delete;
    ...
    private int getDistance(String s0, String s1) {
        ...
        for (int i=1; i<=lengthS1; i++) {
            for (int j=1; j<=lengthS0; j++) {
                table[i][j] = Math.max(Math.max(Math.max(table[i][j-1] + delete, 0),
                        table[i-1][j] + insert),
    ...
    private String getDictWord(LocalEditDistance distance, String mistakeWord, ArrayList<String> listDict) {
    ...
    private ArrayList<String> getDictWords(LocalEditDistance distance, String mistakeWord, ArrayList<String> listDict) {
        ...
        for(String result: dicMap.keySet()){
            if(dicMap.get(result).equals(max)) {
    ...
    public static void main(String args[]) throws FileNotFoundException {
        LocalEditDistance distance = new LocalEditDistance(1, -1, -1, -1);
